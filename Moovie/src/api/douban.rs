...
pub async fn fetch_douban_search_subjects(
    ...
    let data: DoubanSearchResponse = serde_json::from_str(&text)
        .map_err(|e| format!("解析JSON失败: {}, 响应: {}", e, text))?;
...
pub async fn fetch_douban_new_search(
    ...
    let url = reqwest::Url::parse_with_params("https://movie.douban.com/j/new_search_subjects", &params)
    ...
    let new_data: DoubanNewSearchResponse = serde_json::from_str(&text)
        .map_err(|e| format!("解析JSON失败: {}, 响应: {}", e, text))?;
...
pub async fn douban_chart_top_list(
    ...
    let interval_id = params.get("interval_id").cloned().unwrap_or("100:90".to_string());
    ...
    let url = match reqwest::Url::parse_with_params(
        ...
        Err(e) => {
            return (
                ...
                Json(ApiResponse::<DoubanSearchResponse>::error(&format!("构建URL失败: {}", e))),
            );
        }
    };
        ...
        .await
    {
        ...
        Err(e) => {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ApiResponse::<DoubanSearchResponse>::error(&format!("请求失败: {}", e))),
            );
        }
    };
    ...
    if !status.is_success() {
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(ApiResponse::<DoubanSearchResponse>::error(&format!("HTTP 错误: {}", status))),
        );
    }
    ...
    let text = match response.text().await {
        ...
        Err(e) => {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ApiResponse::<DoubanSearchResponse>::error(&format!("读取响应失败: {}", e))),
            );
        }
    };
    ...
    let subjects: Vec<DoubanSubject> = match serde_json::from_str(&text) {
        ...
        Err(e) => {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ApiResponse::<DoubanSearchResponse>::error(&format!("解析JSON失败: {}", e))),
            );
        }
    };
