...
''';
  ...
  static String getAbogusUrl(String url, String userAgent) {
    final flutterJs = getJavascriptRuntime(xhr: false);
    final msToken = generateMsToken(107);
    var params = ('$url&msToken=$msToken').split('?')[1];
    var query = params.contains("?") ? params.split("?")[1] : params;
    var jsCode = kABogus;
    flutterJs.evaluate(jsCode);
    // 执行getABogus函数
    var aBogus =
        flutterJs.evaluate("getABogus('$query', '$userAgent')").stringResult;
    flutterJs.dispose();
    var newUrl =
  ...
  static String getSignature(String roomId, String uniqueId) {
    ...
    flutterJs.evaluate(kWebMsSDK);
    var msStub = getMsStub(roomId, uniqueId);
    var signature = flutterJs
        .evaluate("getMSSDKSignature('$msStub','$defaultUserAgent')")
        .stringResult;
